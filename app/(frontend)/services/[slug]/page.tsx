...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <PageHero eyebrow={`${category.label} · Kathmandu, Nepal`} title={service.title} description={service.description} path={`/services/${service.slug}`} label={service.shortTitle} parent={{ label: "Services", href: "/services" }} category={category}><Link className="hero-cta" href={`/contact?service=${encodeURIComponent(service.title)}`}>Discuss this service <ArrowRight aria-hidden="true" /></Link></PageHero>
