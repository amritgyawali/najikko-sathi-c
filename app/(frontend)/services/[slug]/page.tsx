...
import { SectionScope } from "../../_components/section-scope";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!service) notFound();
  return await pageMetadata(`${service.title} in Nepal`, service.metaDescription, `/services/${service.slug}`);
}
...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    {/* A service page composes the same bands a built-in page is built from,
        so each is marked with what it is and Site Settings → Section styles
        reaches it exactly as it reaches the rest of the site. */}
    <SectionScope kind="pageHero"><PageHero eyebrow={`${category.label} · Kathmandu, Nepal`} title={service.title} description={service.description} path={`/services/${service.slug}`} label={service.shortTitle} parent={{ label: "Services", href: "/services" }} category={category} mediaKey={service.slug}><Link className="hero-cta" prefetch={false} href={`/contact?service=${encodeURIComponent(service.title)}`}>Discuss this service <ArrowRight aria-hidden="true" /></Link></PageHero></SectionScope>
    <SectionScope kind="prose"><section className="content-section"><div className="site-container service-overview"><div className="prose"><SectionHeading kicker="The purpose" title={`A closer look at ${service.shortTitle.toLowerCase()}`} /><p className="lead-copy">{service.intro}</p>{service.audience ? <><h3>Who this is for</h3><p>{service.audience}</p></> : null}{service.preparation ? <><h3>What to prepare</h3><p>{service.preparation}</p></> : null}</div><aside className="scope-card"><span className="section-kicker">The scope</span><h2>What we can work on</h2><ul>{service.deliverables.map((item) => <li key={item}><Check aria-hidden="true" /><span>{item}</span></li>)}</ul><p>Final deliverables, timing, and fees are confirmed in your project proposal.</p><Link className="text-link" href={category.href}>Explore {category.label.toLowerCase()} <ArrowRight aria-hidden="true" /></Link></aside></div></section></SectionScope>
    {service.steps.length > 0 ? <SectionScope kind="processSteps"><section className="content-section process-section"><div className="site-container"><SectionHeading kicker="How it comes together" title="A clear path from brief to outcome." /><ProcessSteps steps={service.steps} /></div></section></SectionScope> : null}
    <SectionScope kind="mediaShowcase"><MediaShowcase mediaKey={service.slug} title={service.shortTitle} service={service.shortTitle} /></SectionScope>
    {service.faq.length > 0 ? <SectionScope kind="faqSection"><section className="content-section"><div className="site-container faq-grid"><SectionHeading kicker="Before you begin" title="Useful things to know." /><Questions items={service.faq} /></div></section></SectionScope> : null}
    {related.length > 0 ? <SectionScope kind="serviceCards"><section className="content-section related-section"><div className="site-container"><SectionHeading kicker="Keep exploring" title="Related services" /><ServiceCards services={related} /></div></section></SectionScope> : null}
    <SectionScope kind="contactCta"><ContactCta service={service.title} title={`Let’s talk about ${service.shortTitle.toLowerCase()}.`} /></SectionScope>
    <StructuredData data={[
