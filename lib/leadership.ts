 ...
 *
 * `name` is the company rather than a person: the messages were supplied
 * without signatures. Putting the chairman's and the director's own names into
...
/** The director's message, in English. */
...
/** The chairman's message, in Nepali. */
...
export const leadershipMessages: LeadershipMessageContent[] = [
  ...
  },
  {
    ...
    photoUrl: null,
    photoAlt: `अध्यक्ष, ${COMPANY}`,
  },
];

/** The same messages, shaped for the `leadershipMessages` field in the CMS. */
export const leadershipMessageRows = leadershipMessages.map(({ role, name, heading, message }) => ({
  role,
  name,
  heading,
  message,
